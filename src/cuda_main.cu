...
__constant__ float sobelX[KERNEL_DIMENSION][KERNEL_DIMENSION] = {
    {-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};
__constant__ float sobelY[KERNEL_DIMENSION][KERNEL_DIMENSION] = {
    {-1, -2, -1}, {0, 0, 0}, {1, 2, 1}};

// Do not use global variables

/* CUDA Kernel that applies Sobel's filter */
__global__ void CUDAedgeDetection(const uint8_t *img, uint8_t *buffer,
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    ...
    std::cerr << "Try ./cuda <input.jpg> <output.jpg> <threadsPerBlock> "
                 "<sequential.jpg>\n";
    exit(1);
  }
  ...
  if (threadsPerBlock <= 0) {
    ...
    std::cerr << "Argument <threadsPerBlock> should be a positive integer "
                 "bigger than 0\n";
    exit(1);
  }
  ...
  dim3 blkCnt(ceil(width / threadsPerDimension),
              ceil(height / threadsPerDimension), 1);
  /* Assign GRID_SIZE size chunks for each SP */
  dim3 thPerBlk(threadsPerDimension, threadsPerDimension, 1);

  std::cout << "SM blocks queued up: " << blkCnt.x * blkCnt.y
            << ". SP threads queued up: " << thPerBlk.x * thPerBlk.y
            << ". Total thrads queued up: "
            << blkCnt.x * blkCnt.y * thPerBlk.x * thPerBlk.y << std::endl;

  /* Start the timer */
  Timer t;

  /* Call CUDA Kernel */
  CUDAedgeDetection<<<blkCnt, thPerBlk>>>(cuda_input, cuda_output, width,
                                          height);
